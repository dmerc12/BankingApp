...
def test_service_create_account_customer_id_not_integer():
    try:
        test_account = BankAccount(successful_account.account_id, "this won't work", successful_account.balance)
...
def test_service_create_account_balance_not_float():
    try:
        test_account = BankAccount(successful_account.account_id, successful_account.customer_id, "this won't work")
        account_sao.service_create_account(test_account)
        assert False
    except FailedTransaction as error:
...
def test_service_get_account_by_id_not_integer():
    try:
...
def test_service_get_all_accounts_customer_id_not_integer():
    try:
...
def test_service_deposit_account_id_not_integer():
    try:
        account_sao.service_deposit("this won't work", 25.00)
        assert False
    except FailedTransaction as error:
        assert str(error) == "The account ID field must be an integer, please try again!"
...
def test_service_withdraw_account_id_not_integer():
    try:
        account_sao.service_withdraw("this won't work", 25.00)
        assert False
    except FailedTransaction as error:
        assert str(error) == "The account ID field is must be an integer, please try again!"
...
def test_service_delete_account_id_not_integer():
    try:
